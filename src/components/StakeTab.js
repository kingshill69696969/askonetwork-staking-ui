...
export default function StakeTab({accountAsko,setRequestStakeValue,requestStakeValue,handleStake}) {
  return(
    <Box w="100%" pt="50px" pb="50px" textAlign="center" color="asko.fg" position="relative">
      ...
      <Text w="100%" fontSize={{base:"18px",sm:"21px"}} color="asko.fgMed2"
        ...
        MAX: {removeDecimal(accountAsko)}
      </Text>
      <NumberInput fontSize={{base:"18px",sm:"21px"}}  w={{base:"100%",md:"700px"}}
        display="inline-block" value={removeDecimal(requestStakeValue)} min={0}
        max={removeDecimal(accountAsko)} step={1}
        >
        ...
        fontSize={{base:"18px",sm:"21px"}} position="relative" zIndex="1"
          ...
          onChange={e => {
            ...
            if(e.target.value === "") {
              setRequestStakeValue("")
            } else if(Number(e.target.value) > 140000000) {
              setRequestStakeValue("140000000")
            } else if(Number(e.target.value) < 1) {
              setRequestStakeValue("1")
            } else{
              setRequestStakeValue(removeDecimal(Number(e.target.value)))
            }
          }} />
        <Button fontSize={{base:"18px",sm:"21px"}} display={{base:"block",sm:"inline-block"}}
          ...
          onClick={()=>setRequestStakeValue(removeDecimal(accountAsko))}>
          Max
        </Button>
      </NumberInput>
