  ...
  return(
    <Box m="60px" ml="auto" mr="auto" textAlign="center">
      <NumberInput clampValueOnBlur={true} display="inline-block" value={val} min={0} max={5} precision={2} w="100px" m="20px" ml="auto" mr="auto" color="gray.700" >
        ...
        <NumberInputStepper>
        <NumberIncrementStepper onClick={()=>{(val<cap) ? setVal(val+1) : setVal(cap)}} />
        <NumberDecrementStepper onClick={()=>{(val>=1) ? setVal(val-1) : setVal(0)}} />
      </NumberInputStepper>
      </NumberInput>
      <Button fontSize="xs" color="gray.300" display="inline-block"  bg="gray.700" fg="gray.300" p="0px" h="25px" w="25px" m="10px" minWidth="0px" mb="12px" onClick={()=>setVal(cap)}>ᐱ</Button>
